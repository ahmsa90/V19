...
const FAVORITES_KEY = 'ussdFavoriteBeneficiaries';
...
const favoritePhoneLabel = document.getElementById('favoritePhoneLabel');
...
const paymentConfirmCancelButton = document.getElementById('paymentConfirmCancel');
...
let activeRetryRecordId = '';
let activeRetryOriginalAmount = '';
let activeDetailsEditRecordId = '';
let favoriteCandidatePhone = '';
...
function getFavoriteByPhone(phone) {
  return loadFavorites().find(item => item.phone === phone);
}

function updateFavoriteStarVisibility() {
  const phone = normalizePhone(phoneInput.value);
  const show = validatePhone(phone) && !selectedContactName && !getFavoriteByPhone(phone);
  addFavoriteButton.hidden = !show;
}

function renderFavoriteContactMessage(name) {
  contactSuccess.innerHTML = `سيتم التحويل إلى <strong>${escapeHtml(name)}</strong> <span class="favorite-inline-star">★</span>`;
}
...
amountInput.addEventListener('input', () => {
  amountInput.classList.remove('input-invalid');
  if (activeRetryRecordId && activeRetryOriginalAmount && Number(normalizeAmount(amountInput.value)) !== Number(activeRetryOriginalAmount)) {
    setFieldError(amountError, 'أنت على وشك تغيير المبلغ!');
  } else {
    setFieldError(amountError, '');
  }
  generateCode();
...
clearFieldsButton.addEventListener('click', () => {
  ...
  selectedContactName = '';
  setRetryPaymentRestrictions(false);
  updateFavoriteStarVisibility();
  contactSuccess.textContent = '';
  setFieldError(phoneError, '');
  setFieldError(amountError, '');
...
function extractTransferPayloadFromQr(text = '') {
  const raw = normalizeArabicDigits(String(text || ''));
  const ussd = raw.match(/\*370\*1\*1\*([^*#]+)\*([0-9٠-٩۰-۹.]+)#?/);
  let phone = '';
  let amount = '';

  if (ussd) {
    phone = normalizePhone(ussd[1]);
    amount = normalizeAmount(ussd[2]);
  }

  if (!validatePhone(phone)) {
    const candidates = raw.match(/(?:\+?970|\+?972|00970|00972|0)?5[69]\d{7}/g) || [];
  ...
  if (!amount) {
    const amountMatch = raw.match(/\*([0-9٠-٩۰-۹.]+)#?$/);
    if (amountMatch) amount = normalizeAmount(amountMatch[1]);
  }
...
async function applyQrText(text) {
  const payload = extractTransferPayloadFromQr(text);
  if (!validatePhone(payload.phone)) {
    showToast('لم يتم العثور على رقم جوال صالح داخل الكود.');
    return false;
  }
  await handlePickedPhone(payload.phone, '');
  if (payload.amount && validateAmount(payload.amount)) {
    amountInput.value = payload.amount;
  }
  showToast('تمت قراءة بيانات QR.');
  return true;
}

async function startQrScanner() {
  if (!navigator.mediaDevices?.getUserMedia) {
    showToast('الكاميرا غير مدعومة على هذا الجهاز.');
    return;
  }
  if (!('BarcodeDetector' in window)) {
    showToast('ماسح QR غير مدعوم في هذا الإصدار من WebView.');
    return;
  }
  try {
    ...
    const scan = async () => {
      try {
        if (!qrScannerModal.hidden && qrVideo.readyState >= 2) {
          const codes = await detector.detect(qrVideo);
          if (codes?.length) {
            const applied = await applyQrText(codes[0].rawValue || '');
            if (applied) {
              await stopQrScanner();
              return;
            }
    ...
    await stopQrScanner();
    showToast('تعذر فتح الكاميرا أو لم يتم منح الصلاحية.');
  }
}
...
async function handlePickedPhone(rawPhone, rawName = '') {
  ...
  const favorite = selectedContactName ? null : getFavoriteByPhone(normalized);
  if (favorite) selectedContactName = favorite.name;
  if (selectedContactName) {
    if (favorite) renderFavoriteContactMessage(selectedContactName);
    else contactSuccess.innerHTML = `سيتم التحويل إلى <strong>${escapeHtml(selectedContactName)}</strong>`;
  } else {
...
function completeTransferRecord(payload) {
  let record = activeRetryRecordId ? records.find(item => item.id === activeRetryRecordId) : null;

  if (record && record.status === 'unpaid') {
    record.phone = payload.phone;
    record.amount = Number(payload.amount);
    record.code = payload.code;
    record.wallet = payload.wallet;
    record.contactName = String(payload.contactName || '').trim();
    record.details = String(payload.details || '').trim();
    record.status = 'unpaid';
    saveRecords();
  } else {
    record = createRecord(payload.phone, payload.amount, payload.code, payload.wallet, payload.contactName, payload.details);
  }
...
paymentConfirmYesButton.addEventListener('click', async () => {
  ...
  activeRetryRecordId = '';
  activeDetailsEditRecordId = '';
  setMainFieldsLocked(false);
  setRetryPaymentRestrictions(false);
  setTransferButtonMode('pay');
  clearPendingPaymentConfirmation();
...
paymentConfirmCancelButton.addEventListener('click', async () => {
  const recordId = pendingPaymentConfirmationRecordId;
  const record = records.find(item => item.id === recordId);
  if (record && activeRetryRecordId !== record.id) {
    await cancelRecordReminder(record);
  ...
  setTransferButtonMode('pay');
  clearPendingPaymentConfirmation();
  showToast('لم يتم إضافة العملية إلى السجل');
});
...
callButton.addEventListener('click', async () => {
  if (callButton.dataset.mode === 'edit' && activeDetailsEditRecordId) {
    const record = records.find(item => item.id === activeDetailsEditRecordId);
    if (record) {
      record.details = movementDetailsInput.value.trim();
      saveRecords();
      renderRecords();
    }
    activeDetailsEditRecordId = '';
    ...
    showToast('تم حفظ تفاصيل الحركة.');
    return;
  }
  ...
  if (activeRetryRecordId && activeRetryOriginalAmount && Number(amount) !== Number(activeRetryOriginalAmount)) {
    setFieldError(amountError, 'أنت على وشك تغيير المبلغ!');
  }
  ...
  if (!activeRetryRecordId && isConsecutiveDuplicate(phone, amount, selectedWallet, details)) {
...
addFavoriteButton.addEventListener('click', () => {
  const phone = normalizePhone(phoneInput.value);
  if (!validatePhone(phone)) {
    showToast('أدخل رقم جوال صحيح أولاً.');
    return;
  }
  favoriteCandidatePhone = phone;
  favoritePhoneLabel.textContent = phone;
  favoriteNameInput.value = '';
  favoriteNameModal.hidden = false;
...
saveFavoriteButton.addEventListener('click', () => {
  ...
  if (!name) {
    showToast('أدخل اسم المستفيد.');
    return;
  }
  const list = loadFavorites().filter(item => item.phone !== favoriteCandidatePhone);
  list.unshift({ phone: favoriteCandidatePhone, name, createdAt: new Date().toISOString() });
  saveFavorites(list);
  selectedContactName = name;
  renderFavoriteContactMessage(name);
  favoriteNameModal.hidden = true;
  updateFavoriteStarVisibility();
  showToast('تمت إضافة الرقم إلى المفضلة.');
});
...
function renderFavoritesList() {
  const search = String(favoritesSearchInput.value || '').trim().toLowerCase();
  const list = loadFavorites().filter(item => {
    const text = `${item.name} ${item.phone}`.toLowerCase();
    return !search || text.includes(search);
  });
  favoritesList.innerHTML = list.length ? list.map(item => `
    <button type="button" class="favorite-row" data-phone="${item.phone}" data-name="${escapeHtml(item.name)}">
      <strong>${escapeHtml(item.name)}</strong>
      <span>${item.phone}</span>
    </button>
  `).join('') : '<p class="empty-favorites">لا توجد أسماء مفضلة</p>';
}

phoneInput.addEventListener('focus', () => {
  updateFavoriteStarVisibility();
  const list = loadFavorites();
  if (!list.length || selectedContactName) return;
  renderFavoritesList();
  favoritesModal.hidden = false;
});

favoritesSearchInput.addEventListener('input', renderFavoritesList);
closeFavoritesButton.addEventListener('click', () => { favoritesModal.hidden = true; });
favoritesList.addEventListener('click', async event => {
  const row = event.target.closest('.favorite-row');
  if (!row) return;
  await handlePickedPhone(row.dataset.phone, row.dataset.name);
...
function setMainFieldsLocked(locked) {
  phoneInput.disabled = locked;
  amountInput.disabled = locked;
  phoneInput.classList.toggle('field-locked', locked);
  amountInput.classList.toggle('field-locked', locked);
}


function setRetryPaymentRestrictions(enabled, originalAmount = '') {
  pickContactButton.disabled = Boolean(enabled);
  scanQrButton.disabled = Boolean(enabled);
  phoneInput.readOnly = Boolean(enabled);
  activeRetryOriginalAmount = enabled ? String(originalAmount || amountInput.value || '') : '';

  pickContactButton.classList.toggle('locked-action', Boolean(enabled));
  scanQrButton.classList.toggle('locked-action', Boolean(enabled));
  phoneInput.classList.toggle('field-locked', Boolean(enabled));
}


function updateTransferWalletIcon(show = false) {
  let icon = callButton.querySelector('.transfer-wallet-icon');
  if (!icon) {
    icon = document.createElement('img');
    icon.className = 'transfer-wallet-icon';
    icon.alt = '';
    callButton.prepend(icon);
  }
  icon.src = selectedWallet === 'jawwal' ? 'icons/jawwalpay-logo-clean.png' : 'icons/palpay-logo-clean.png';
  icon.hidden = !(show && validatePhone(normalizePhone(phoneInput.value)) && validateAmount(normalizeAmount(amountInput.value)));
}

function setTransferButtonMode(mode = 'pay') {
  callButton.classList.toggle('retry-transfer-mode', mode === 'retry');

  if (mode === 'edit') {
    callButton.dataset.mode = 'edit';
    callButton.innerHTML = `
      <span>حفظ التفاصيل</span>
      <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 20h14a1 1 0 0 0 1-1V8.8a1 1 0 0 0-.29-.71l-3.8-3.8A1 1 0 0 0 15.2 4H5a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1Zm3-2v-5h8v5H8Zm0-12h6v4H8V6Z"/></svg>
    `;
  } else {
    callButton.dataset.mode = 'pay';
    callButton.innerHTML = `
      <span>التحويل</span>
      <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1C10.61 21 3 13.39 3 4c0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.24.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2Z"/></svg>
    `;
  }
}

async function restoreRecordForDetailsEdit(recordId) {
  const record = records.find(item => item.id === recordId);
  if (!record) return;

  activeDetailsEditRecordId = record.id;
  activeRetryRecordId = '';
  selectedWallet = record.wallet === 'jawwal' ? 'jawwal' : 'palpay';
  localStorage.setItem(WALLET_KEY, selectedWallet);
  updateWalletButtons();

  phoneInput.value = record.phone || '';
  amountInput.value = String(record.amount ?? '');
  movementDetailsInput.value = record.details || '';
  selectedContactName = String(record.contactName || '').trim();
  ...
  setMainFieldsLocked(true);
  setTransferButtonMode('edit');
  generateCode();
  await showView('transfer');
  movementDetailsInput.focus();
  showToast('يمكنك تعديل تفاصيل الحركة فقط.');
}

async function restoreRecordForPayment(recordId) {
  const record = records.find(item => item.id === recordId && item.status === 'unpaid');
  if (!record) return;

  activeRetryRecordId = record.id;
  activeRetryOriginalAmount = String(record.amount ?? '');
  activeDetailsEditRecordId = '';
  setMainFieldsLocked(false);
  setRetryPaymentRestrictions(true, record.amount);
  setTransferButtonMode('retry');
  selectedWallet = record.wallet === 'jawwal' ? 'jawwal' : 'palpay';
  localStorage.setItem(WALLET_KEY, selectedWallet);
  updateWalletButtons();
  ...
  generateCode();
  await showView('transfer');
  showToast('تمت استعادة بيانات الحركة للدفع.');
}
...
function renderRecords() {
  ...
  filtered.forEach((record, index) => {
    ...
    tr.innerHTML = `
      ...
      <td class="phone-cell ${record.status === 'unpaid' ? 'retry-enabled' : ''}">
        ...
        ${record.status === 'unpaid' ? `
          <button class="retry-pay-action" type="button" data-id="${record.id}" title="الذهاب للدفع" aria-label="الذهاب للدفع">
            <span class="retry-pay-bubble" aria-hidden="true">
              <svg viewBox="0 0 24 24"><path d="M4 7.5A2.5 2.5 0 0 1 6.5 5h11A2.5 2.5 0 0 1 20 7.5v9a2.5 2.5 0 0 1-2.5 2.5h-11A2.5 2.5 0 0 1 4 16.5v-9Zm0 2.2h16M8 15h4m5-4 3 3-3 3m3-3h-6"/></svg>
            </span>
            <span class="retry-pay-label">دفع</span>
          </button>
        ` : ''}
      </td>
      ...
      <td>
        <div class="details-display editable">
          <span class="details-text">${record.details ? escapeHtml(record.details) : ''}</span>
          <button class="edit-details-action" type="button" data-id="${record.id}" title="تعديل التفاصيل" aria-label="تعديل التفاصيل">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 16.5 9.9-9.9 3.5 3.5-9.9 9.9H4v-3.5Zm11-11 1.6-1.6a2 2 0 0 1 2.8 0l.7.7a2 2 0 0 1 0 2.8L18.5 9 15 5.5Z"/></svg>
          </button>
        </div>
      </td>
      <td>
...
function resizeDetailsInput(textarea) {
  ...
  const previousHeight = textarea.style.height || '34px';

  if (isEmpty) {
    textarea.style.height = '34px';
  } else {
    // Avoid changing height while the text still fits on one line.
    textarea.style.height = '34px';
    const needsMoreThanOneLine = textarea.scrollHeight > 38;
    if (needsMoreThanOneLine) {
  ...
  // Some Android WebView keyboards move the caret to the start when height changes.
  // Restore it so Arabic letters are appended in the correct order.
...
function bindRecordEvents() {
  ...
  document.querySelectorAll('.edit-details-action').forEach(button => {
    button.addEventListener('click', async e => {
      await restoreRecordForDetailsEdit(e.currentTarget.dataset.id);
    });
  });

  document.querySelectorAll('.retry-pay-action').forEach(button => {
    button.addEventListener('click', async e => {
      await restoreRecordForPayment(e.currentTarget.dataset.id);
    });
  });
...
async function handlePaymentNotificationAction(action) {
  ...
  if (actionId === PAYMENT_NOTIFICATION_UNPAID_ACTION) {
    ...
    renderRecords();
    updatePaidTotal(getFilteredRecords());
    await restoreRecordForPayment(record.id);
    showToast('بقيت الحركة غير مدفوعة وتمت استعادة بياناتها للدفع.');
    return;
  }
...
registerSystemBackButtonListener();
updateWalletButtons();
renderRecords();
generateCode();
